...
class _CardInputViewState extends State<CardInputView> {
  ...
  Future<void> _onOnlineRequested(EmvOnlineRequestedEvent event) async {
    ...
    if (transactionArgs != null) {
      ...
      final PaymentRepository payment = PaymentRepositoryImpl(datasource: PaymentHostDatasourcePharos());
      final BinResponseEntity? binMsi = await payment.getAvailableMsi(transactionArgs);
      if (binMsi != null && binMsi.listMsi.isNotEmpty) {
        print("Vamos a ver si ya se genero bien los MSI");
        binMsi.toString();
      }
      print("A ver a ver que paso");
      // transactionArgs.msi =  
