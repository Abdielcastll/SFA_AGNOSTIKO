...
int numImagesLimit = Platform.isLinux ? 1 : 10;
...
Future<PrintImagesResult> printToImages(
    List<PrinterObject> printerObjects, int elementIndex) async {
  List<ui.Image> images = [];
  int i = elementIndex;
  ...
  do {
    final imageResult = await _printToImage(printerObjects, i);
    if (images.length < numImagesLimit) {
...
Future<_PrintImageResult> _printToImage(
    List<PrinterObject> printerObjects, int index) async {
  double offsetY = 0;

  _PrintResult printResult = _PrintResult(0, false);
  int elementIndex = 0;

  final pixels = await getPaperWidth();
...
Future<Uint8List> printToBytes(
    PrinterScript printerScript, PrinterImageFormat format) async {
  List<PrinterObject> printerObjects = printerScript.printObjects;
  if (printerObjects.isEmpty) {
    return Uint8List.fromList([]);
  }

  final result = await printToImages(printerObjects, 0);
  late ByteData? data;
...
///
/// Nota: Se recomienda la división de textos de grandes longitudes, mediante la
/// generación de múltiples PrinterScript de dimensiones moderadas e imprimirlos
/// sucesivamente para el armado de tickets largos.
Future<void> printScript(PrinterScript printerScript,
    {bool bottomFeed = false}) async {
  ...
  do {
    ...
    int allHeights = 0;
    int width = 0;
    final imageResult = await printToImages(printerObjects, i);
    i = imageResult.elementIndex;

    for (var img in imageResult.images) {
      final byteData = await img.toByteData(format: ui.ImageByteFormat.rawRgba);
      allHeights = allHeights + img.height;
      width = img.width;
    ...
    if (i == (printerObjects.length - 1)) {
      await printBitmap(rgba, width, allHeights, printerScript.gray,
...
Future<void> printBitmap(
    Uint8List bitmapRGBA, int width, int height, GrayIntensity gray,
    {bool bottomFeed = false}) async {
